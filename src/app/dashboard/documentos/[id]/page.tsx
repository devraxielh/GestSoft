...
interface Documento {
    ...
    presentacionFacultad: string | null
    presentacionPrograma: string | null
...
export default function DocumentoEditorPage() {
    ...
    const handleToggleStatus = async () => {
        if (!doc) return
        const newStatus = doc.status === "Terminado" ? "En construcción" : "Terminado"
        setSaving(true)
        try {
            const res = await fetch(`/api/documentos/${id}`, {
                method: "PUT",
            ...
            if (res.ok) {
                ...
                toast.success(`Documento ${newStatus.toLowerCase()}`)
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-sm">
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button onClick={handleToggleStatus} disabled={saving} className={`inline-flex items-center gap-2 rounded-lg px-5 py-2.5 text-sm font-medium shadow-theme-xs transition-colors disabled:opacity-50 ${doc.status === "Terminado" ? "bg-amber-100 text-amber-700 hover:bg-amber-200" : "bg-green-100 text-green-700 hover:bg-green-200"}`}>
                            {saving ? <div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin"></div> : (
                                doc.status === "Terminado"
                                    ? <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" /></svg>
                                    : <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                            )}
                            {doc.status === "Terminado" ? "Reabrir documento" : "Terminar documento"}
                        </button>
